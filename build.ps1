...
$license_output = Join-Path $windows_x64_release_dir "license"
Copy-Item -Path $license_dir -Destination $license_output -Recurse

# zip windows x64
$zip_output = Join-Path $release_dir "windows_x64.zip"
$zip_files = Join-Path $windows_x64_release_dir "*"
Compress-Archive -Path $zip_files -DestinationPath $zip_output
...
if(!$?) { Exit $LASTEXITCODE }
$license_output = Join-Path $linux_release_dir "license"
Copy-Item -Path $license_dir -Destination $license_output -Recurse
